...
// HigherOrder ダメポ, Inference もダメポ (toplevel val は intercept しないの忘れてた…)
// Return も仕様にマッチしていないようでダメポ
// trycatch0:  shift/reset に使っている Throwable でキャッチされる…
...
class IfReturn extends JUnitSuite {
  ...
  @Test def shift_pct = {
    ...
    1 to 25 foreach { i =>
      ...
      inverse_macros.continuations.run[Double](irr p i)
    }
  }
